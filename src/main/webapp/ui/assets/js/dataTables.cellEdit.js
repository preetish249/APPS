...
jQuery.fn.dataTable.Api.register('MakeCellsEditable()', function (settings) {
    ...
    var editRow,editRowObj,currRow;
    jQuery.fn.extend({
        // UPDATE
        updateEditableCell: function (callingElement) {
            ...
            if(inputField.attr("type") == "checkbox"){
                if(inputField.prop("checked"))
                    newValue = inputField.attr("checkedVal");
                else
                    newValue = inputField.attr("uncheckedVal");
            }
    ...
    if (table != null) {
        ...
        $(table.body()).on('dblclick', 'tr', function () {
            editRow = table.cell($(this).children("td")[0]).index().row;
            editRowObj = $(this);        
            var tds = $(this).children("td");
            $.each(tds,function(index, pThis , val){
        ...
        $(table.body()).on('click', 'tr', function (evt) {
            currRow = table.cell($(this).children("td")[0]).index().row;
            if(editRow != undefined && editRow != currRow){
                var tds = editRowObj.children("td");
                $.each(tds,function(index, pThis , val){
                    if($(pThis).children('input').length > 0){
                        let inpElm = $(pThis).children('input')[0];
                        $(inpElm).updateEditableCell(inpElm);
                    }else if($(pThis).children('select').length > 0){
                        let ddnObj = $(pThis).children('select')[0];
                        $(ddnObj).updateEditableCell(ddnObj);
                    }
                })
            }
        });
...
function getInputHtml(currentColumnIndex, settings, oldValue) {
    ...
    switch (inputType) {
        case "list":
            input.html = startWrapperHtml + "<select class='select2init " + inputCss + "'>";
        ...
        case "datepicker-confirm":
	        input.html = startWrapperHtml + "<input id='ejbeatycelledit_dp' type='text' name='date' class='datepicker " + inputCss + "'   value='" + oldValue + "'></input>" + endWrapperHtml;
	        setTimeout(function () { //Set timeout to allow the script to write the input.html before triggering the datepicker
	            
                let appDateFormat = this.xelerate.appConfig.dateFormat;
                let inpDateFormat = inputSetting.options.dateformat ? inputSetting.options.dateformat : 'd/m/y';
                let defaultParams = {
                ...
                let finalParams = {
                    ...defaultParams,
                    ...inputSetting.options.userSettings
                }

                var self = flatpickr(jQuery('.datepicker')[1],finalParams);
	        },100);
	        break;
        ...
        case "textarea-confirm":
            ...
            break;
        case "checkbox":
            if(inputSetting.options.checked == oldValue){
                input.html = startWrapperHtml + "<input id='ejbeatycelledit' type='checkbox' checked checkedVal = '"+inputSetting.options.checked+"' uncheckedVal = '"+inputSetting.options.unchecked+"' class='" + inputCss + "'></input>" + endWrapperHtml;
            }else{
                input.html = startWrapperHtml + "<input id='ejbeatycelledit' type='checkbox' checkedVal = '"+inputSetting.options.checked+"' uncheckedVal = '"+inputSetting.options.unchecked+"' class='" + inputCss + "'></input>" + endWrapperHtml;  
            }
	    ...
	    case "number-confirm" :
            ...
            break; 
        case "number" :
            input.html = startWrapperHtml + "<input id='ejbeatycelledit' type='number' class='" + inputCss + "' value='"+oldValue+"'" + (listenToKeys ? " onkeyup='if(event.keyCode==13) {$(this).updateEditableCell(this);} else if (event.keyCode===27) {$(this).cancelEditableCell(this);}'" : "") + "></input>" + endWrapperHtml;
            break;
        default: // text input
            input.html = startWrapperHtml + "<input id='ejbeatycelledit' class='" + inputCss + "' value='" + oldValue + "'></input>" + endWrapperHtml;
            break;
    }
    return input;
}
